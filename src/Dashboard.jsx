...
import { 
  Search, Menu, Plus, ChevronDown, MoreHorizontal,
  ArrowUpRight, Filter, Download, Share2
} from 'lucide-react';
import Sidebar from './components/Sidebar';
import { kpiData, platformStats } from './data';
        ...
        <header className="flex flex-col-reverse md:flex-row justify-between md:items-center gap-4 mb-8">
          ...
          <div className="relative w-full md:w-96 group">
            ...
            <input 
              ...
              className="w-full bg-gray-50 hover:bg-gray-100 focus:bg-white rounded-full py-3 pl-12 pr-4 text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-gray-100 focus:shadow-md"
            />
          </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6 mb-10">
          ...
          <div className="flex flex-wrap items-center gap-3">
             <div className="flex items-center gap-2 bg-gray-50 border border-gray-100 px-4 py-2 rounded-full text-sm cursor-pointer hover:bg-white hover:shadow-sm transition-all">
               ...
               <span className="font-bold ml-2">Sep 1 - Nov 30, 2023</span>
               <ChevronDown size={14} />
             </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 pb-8">
          ...
          <div className="col-span-1 md:col-span-2 lg:col-span-5 flex flex-col justify-between min-h-[180px]">
             <div>
               ...
               <div className="flex flex-wrap items-baseline gap-3 mb-2">
                 ...
                 <div className="flex gap-2">
                    <span className="bg-pink-100 text-[#F43F5E] px-2 py-0.5 rounded-full text-xs font-extrabold flex items-center">
                    <ArrowUpRight size={12} className="mr-1 stroke-[3]"/> 7.9%
                    </span>
                    <span className="bg-[#F43F5E] text-white px-2 py-0.5 rounded-full text-xs font-bold">
                    $27,335.09
                    </span>
                 </div>
               </div>
             ...
             <div className="flex gap-4 mt-6 overflow-x-auto pb-2">
                <StatBadge label="$209,633" />
                <StatBadge label="$156,841" highlight />
                <StatBadge label="$117,115" />
             </div>
          </div>
          ...
          <div className="col-span-1 lg:col-span-2 bg-white border border-gray-100 rounded-3xl p-5 shadow-sm hover:shadow-md transition-shadow cursor-default group">
            ...
            <div className="flex justify-between items-center bg-gray-50 rounded-full p-1.5 pr-3 cursor-pointer hover:bg-gray-100 transition-colors">
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-xs font-bold">Mikasa</span>
              </div>
              <ChevronDown size={14} className="text-gray-400" />
            </div>
          </div>

          {/* 3. Best Deal (Dark Card) */}
          <div className="col-span-1 md:col-span-2 lg:col-span-3 bg-[#0F172A] text-white rounded-3xl p-6 shadow-xl relative overflow-hidden group hover:scale-[1.01] transition-transform duration-300">
            ...
            <div className="flex justify-between items-start mb-2 relative z-10">
              <p className="text-xs font-medium text-gray-400">Best deal</p>
              <div className="text-xs text-gray-500 border border-gray-700 rounded p-1">★</div>
            </div>
            <h3 className="text-3xl font-bold mb-1 relative z-10">$42,300</h3>
            <p className="text-sm text-gray-400 mb-4 font-medium relative z-10">Rolf Inc.</p>
          ...
          {/* 4. Small KPI Columns */}
          <div className="col-span-1 md:col-span-2 lg:col-span-2 flex flex-row lg:flex-col gap-4">
          ...
          {/* 5. Filters List (MAPPED FROM DATA) */}
          <div className="col-span-1 md:col-span-1 lg:col-span-4 bg-gray-50 rounded-3xl p-6 hover:bg-gray-100/50 transition-colors">
            <div className="flex justify-between items-center mb-6">
              <div className="p-2.5 bg-white rounded-xl shadow-sm border border-gray-100 cursor-pointer hover:border-gray-300 transition-colors"><MoreHorizontal size={18}/></div>
              <button className="flex items-center gap-1.5 text-xs font-bold bg-white px-4 py-2 rounded-full shadow-sm border border-gray-100 hover:shadow-md transition-all">Filters <Filter size={12}/></button>
            </div>
            
            <div className="space-y-5">
              {platformStats.map((stat, index) => (
                <ListItem key={index} {...stat} />
              ))}
            </div>
          </div>
          ...
          <div className="col-span-1 md:col-span-1 lg:col-span-4 bg-gray-50 rounded-3xl p-6 relative flex flex-col hover:bg-gray-100/50 transition-colors">
             <div className="flex justify-between items-center mb-auto">
                <div className="p-2 cursor-pointer opacity-50 hover:opacity-100 transition-opacity"><div className="w-5 h-5 border-l-2 border-b-2 border-gray-400"></div></div>
                <button className="flex items-center gap-1.5 text-xs font-bold bg-white px-4 py-2 rounded-full shadow-sm border border-gray-100 hover:shadow-md transition-all">Filters <Filter size={12}/></button>
             </div>
             ...
             <div className="flex justify-center items-end gap-3 sm:gap-6 h-48 mb-4">
               ...
               <div className="w-12 bg-white rounded-t-xl h-[80%] flex justify-center pt-3 text-[#EA4C89] font-black text-sm shadow-sm relative hover:h-[85%] transition-all cursor-pointer group">
                  <div className="absolute -top-10 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">$142k</div>
                  G <div className="absolute top-8 bg-white p-1 rounded-full shadow-md"><div className="w-2.5 h-2.5 rounded-full bg-green-500"></div></div>
               </div>
          ...
          {/* 7. Sales,Revenue List (Right side) */}
          <div className="col-span-1 lg:col-span-4 bg-white p-2 flex flex-col gap-2">
             <div className="grid grid-cols-5 text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1 px-3">
                <div className="col-span-2">Sales</div>
                <div className="text-right">Revenue</div>
                <div className="text-right">Leads</div>
                <div className="text-right">KPI</div>
             </div>
             <SalesRow name="Armin A." rev="$209,633" leads="41" kpi="0.84" />
             <SalesRow name="Mikasa A." rev="$156,841" leads="54" kpi="0.89" active />
             
             <div className="flex gap-2 mt-4 mb-2 flex-wrap">
                <span className="bg-gray-50 px-3 py-1.5 rounded-full text-xs font-bold text-gray-600 border border-transparent hover:border-gray-200 cursor-pointer transition-all">Top sales 👍</span>
                <span className="bg-gray-50 px-3 py-1.5 rounded-full text-xs font-bold text-gray-600 border border-transparent hover:border-gray-200 cursor-pointer transition-all">Sales streak 🔥</span>
             </div>
             ...
             <div className="mt-4 p-5 rounded-3xl bg-gradient-to-br from-pink-50 via-white to-pink-50 border border-pink-100 relative shadow-sm group hover:shadow-md transition-all">
                <div className="flex justify-between items-start mb-8">
                   ...
                   <span className="bg-[#F43F5E] text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-pink-200 shadow-md">+$156,841</span>
                </div>
                
                <h2 className="text-4xl font-bold text-gray-300 mb-1 flex items-baseline gap-2">
                    45.3% <span className="text-xl text-gray-400 font-semibold">$71,048</span>
                </h2>
                
                {/* Line Chart Mock */}
                <div className="h-16 w-full mt-6 flex items-end gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                   <div className="w-full h-[20%] bg-pink-200 rounded-t-sm"></div>
                   <div className="w-full h-[40%] bg-pink-300 rounded-t-sm"></div>
                   <div className="w-full h-[30%] bg-pink-200 rounded-t-sm"></div>
                   <div className="w-full h-[60%] bg-[#F43F5E] rounded-t-sm shadow-lg shadow-pink-200"></div>
                   <div className="w-full h-[45%] bg-pink-300 rounded-t-sm"></div>
                   <div className="w-full h-[55%] bg-pink-300 rounded-t-sm"></div>
                </div>
             </div>
          </div>
          ...
          <div className="col-span-1 md:col-span-1 lg:col-span-4 bg-[#F43F5E] rounded-3xl p-6 text-white flex flex-col justify-between shadow-xl shadow-pink-200 hover:scale-[1.01] transition-transform">
              <div className="flex justify-between items-center text-pink-100 text-xs font-medium">
                 <span>Average monthly</span>
                 <span className="bg-white/10 px-2 py-1 rounded-full backdrop-blur-sm">Platform value</span>
              </div>
              <div className="mt-6 space-y-5">
                 <div className="group cursor-pointer">
                    <p className="text-xs text-pink-200 font-medium group-hover:text-white transition-colors">Revenue</p>
                    <p className="font-black text-2xl tracking-tight">$18,552</p>
                 </div>
                 <div className="w-full h-[1px] bg-white/10"></div>
                 <div className="group cursor-pointer">
                    <p className="text-xs text-pink-200 font-medium group-hover:text-white transition-colors">Leads</p>
                    <p className="font-black text-2xl tracking-tight flex items-baseline gap-2">373 <span className="text-sm font-medium opacity-60">97/276</span></p>
                 </div>
                 <div className="w-full h-[1px] bg-white/10"></div>
                 <div className="group cursor-pointer">
                    <p className="text-xs text-pink-200 font-medium group-hover:text-white transition-colors">Win/lose</p>
                    <p className="font-black text-2xl tracking-tight flex items-baseline gap-2">16% <span className="text-sm font-medium opacity-60">51/318</span></p>
                 </div>
              </div>
          </div>
          ...
          <div className="col-span-1 md:col-span-1 lg:col-span-8 bg-gray-50 rounded-3xl p-6 relative">
             <div className="flex gap-2 justify-end mb-8">
               <span className="bg-black text-white px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wide cursor-pointer hover:bg-gray-800 transition-colors">Revenue</span>
               <span className="text-gray-400 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide cursor-pointer hover:text-gray-600 transition-colors">Leads</span>
               <span className="text-gray-400 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide cursor-pointer hover:text-gray-600 transition-colors">W/L</span>
             </div>
             
             {/* Simple Bar Chart Mock */}
             <div className="flex items-end justify-between h-40 gap-2 sm:gap-4 px-2 sm:px-4">
                <Bar height="h-20" label="Sep" />
                <Bar height="h-28" label="" highlight />
                <Bar height="h-14" label="Oct" />
                <Bar height="h-32" label="" highlight />
                <Bar height="h-24" label="" />
                <Bar height="h-20" label="Nov" />
                <Bar height="h-12" label="" />
                <Bar height="h-16" label="" />
                <Bar height="h-28" label="" highlight/>
             </div>
          </div>
...
const StatBadge = ({ label, highlight }) => (
  <div className={`
    flex items-center gap-2 text-xs font-bold px-1 flex-shrink-0 cursor-pointer transition-colors
    ${highlight ? 'text-blue-500' : 'text-gray-500 hover:text-gray-700'}
  `}>
...
const KPICard = ({ label, value, change, active }) => (
  <div className={`
    flex-1 rounded-[20px] p-4 flex flex-col justify-between items-center transition-all duration-300 hover:-translate-y-1
    ${active ? 'bg-white border-2 border-pink-100 shadow-lg shadow-pink-100/50' : 'bg-gray-50 hover:bg-white hover:shadow-sm'}
  `}>
    ...
    <span className="text-[10px] font-bold text-gray-400 flex items-center">
       {change.includes('+') || !change.includes('-') ? '↑' : '↓'} {change}
    </span>
  </div>
);

const ListItem = ({ icon, name, val, pct, color }) => (
  <div className="flex items-center justify-between group cursor-pointer">
    <div className="flex items-center gap-4">
      <div className={`w-9 h-9 rounded-full bg-white flex items-center justify-center text-sm shadow-sm font-bold ${color} group-hover:scale-110 transition-transform`}>{icon}</div>
...
const SalesRow = ({ name, rev, leads, kpi, active }) => (
  <div className={`
    grid grid-cols-5 items-center py-2.5 px-3 rounded-2xl mb-1 cursor-pointer transition-colors
    ${active ? 'bg-gray-50 shadow-sm' : 'hover:bg-gray-50'}
  `}>
...
const Bar = ({ height, label, highlight }) => (
  <div className="flex flex-col items-center gap-2 w-full group cursor-pointer">
    ...
    <div className={`
        w-full ${height} rounded-t-lg border-x border-t border-dashed relative transition-all duration-300
        ${highlight ? 'bg-gray-200 border-gray-300 group-hover:bg-[#F43F5E]/20' : 'bg-white border-gray-200 group-hover:h-[110%]'} 
    `}>
    </div>
